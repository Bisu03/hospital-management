...
const LabReceipt = () => {
    ...
    return (
        <PrintUi path="/labtest/record">
            <div className="mx-auto bg-white rounded-lg p-4 mt-14">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="text-xs space-y-0.5 print:text-xs">
                            <p><span className="font-semibold">Full Name:</span> {patient?.fullname}</p>
                            <div className="flex space-x-2">
                                <p><span className="font-semibold">Gender:</span> {patient?.gender}</p>
                                <p><span className="font-semibold">Age:</span> {patient?.age}</p>
                            </div>
                            <p><span className="font-semibold">Phone No.:</span> {patient?.phone_number}</p>
                    ...
                    <div>
                        ...
                        <div className="text-xs space-y-0.5 print:text-xs">
                            <p><span className="font-semibold">Date/Time:</span> {formatDate(pathologyData?.reporting_date)}/{pathologyData?.reporting_time}</p>
                            <p><span className="font-semibold">MRD No.:</span> {pathologyData?.mrd_id}</p>
                            <p><span className="font-semibold">Bill No.:</span> {pathologyData?.bill_no}</p>
                            <p><span className="font-semibold">Referred By:</span> {consultant?.drname}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-between space-x-2">
                    <div>
                        <QRCode
                            ...
                            value={`${window.location.href}/labtest/download/${pathologyData?._id}`}
                            viewBox="0 0 100 100"
                        />
                    </div>
                    ...
                    <div className="w-72 space-y-1 text-xs print:text-[10px]">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="text-right">₹{amount?.discount?.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                            <span>Net Total:</span>
                            <span className="text-right">₹{amount?.netTotal?.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="text-right capitalize">{pathologyData?.paydby}</span>
                        </div>
                        <div className="flex justify-between font-semibold border-t pt-1">
                            <span>Due Amount:</span>
                            <span className="text-red-600 text-right">₹{amount?.due?.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
