...
import Radiology from "@/models/Radiology.models";
...
export async function GET(req) {
    try {
        ...
        if (searchParams.id) {
            const data = await Radiology.findById(searchParams.id)
                .populate("patient");
        ...
        const result = await Radiology.aggregate(pipeline);
...
export async function POST(req) {
    try {
        ...
        const newData = await Radiology.create(body);
        await Billing.findOneAndUpdate({ patient: body.patient }, { radiology_cart: body?.test_cart });

        return NextResponse.json({
            success: true,
            message: "Radiology record created",
            data: newData
        });
